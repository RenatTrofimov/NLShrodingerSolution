...
class NonlinearPart():
	def __init__(self):
		...
		self._FF = np.zeros(int(self.mm*self.kk))
		self._A1 = np.zeros(int(self.mm*self.kk))
		self._Gp = np.zeros(int(self.mm*self.kk))
		self._sumFF = np.zeros(int(self.mm*self.kk))
		self._cachedGammaUp = np.empty((int(self.kk), 5))
		self._cachedGammaDown = np.empty((5))
		self.r = np.arange(5)
			
	def getNNP(self, inArray, outArray):
		outArray = np.array([self.GG(element) for element in inArray])
		#outArray = np.array(Parallel(n_jobs=16)(delayed(self.chuck)(element) for element in np.array_split(inArray, 16))).ravel()
	...
	def gammaPart(self, q, U):
		if not np.any(self._cachedGammaUp[int(q)]):
			self._cachedGammaUp[int(q)] = ((-1)**self.r) * (q**(2*self.r + 1))
		if not np.any(self._cachedGammaDown):
	...
	def FF(self, i,k):
        	def func(i,k):
            		return integrate.quad(lambda x :self.tetta(x,i,k)*cos(k*x),-pi,pi, complex_func=True)[0]
        	def func2(i,k):
            		return integrate.quad(lambda x :self.tetta(x,i,k),-pi,pi, complex_func=True)[0]
...
def Besse():
    ...
    while nn < Nt:
        for i in range(M):
            ...
            alfa_plus[i] = jj/r - 2 - lamda * dx**2 * V1[i]
            alfa_minus[i] = -jj/r - 2 - lamda * dx**2 * V1[i]
